...
class Login(HttpClient):
    domain = "ontid_server"
    method = "POST"
    req_url = "web/v1/users/phone-login"
    ...
    headers = {'Secure-Key': 'HFriOp27CnMxnR/6uxH7t9VMRBHReJn0n2v+ZiA+QXjLFZoRZJk5DgM83KRiNbYnKY96kmnjrHghKcjUgK2KLtvgFzQupzWLtnQm2hB49viKT3+c3hvLV0VpeEVbAr/QvjsAFVHJe/5iuvBvMSTVcYYZ766H6xOnrPLzBQi7NTo=',
...
class CheckPhoneExist(HttpClient):
    domain = "ontid_server"
    method = "GET"
    req_url = "web/v1/users/phones/check-existed"
    query_params = ["phone"]
    req_body = {}
    headers = {}
...
class SendResetPasswordVerificationCode(HttpClient):
    domain = "ontid_server"
    method = "POST"
    req_url = "web/v1/users/phone/reset-password-verification-code"
    req_body = {'mode': 'raw',
...
class ResetPassword(HttpClient):
    domain = "ontid_server"
    method = "POST"
    req_url = "web/v1/users/phone-reset-password"
    req_body = {'mode': 'raw',
...
class Register(HttpClient):
    domain = "ontid_server"
    method = "POST"
    req_url = "web/v1/users/phone-register"
    req_body = {
    ...
    headers = {'Content-Type': 'application/ontid.manage.api.v1+json',
               'Accept': 'application/ontid.manage.api.v1+json',
...
class SendRegistrationVerificationCode(HttpClient):
    domain = "ontid_server"
    method = "POST"
    req_url = "web/v1/users/phones/register-verification-code"
    req_body = {"data":
...
class CheckRegistered(HttpClient):
    domain = "ontid_server"
    method = "GET"
    req_url = "web/v1/users/phones/check-registered"
    query_params = ["phone"]
    req_body = {}
    headers = {}
